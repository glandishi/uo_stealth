...
begin
	MoveItem(FindType(Items[1],backpack), 7,Ground, 0, 0, 0);
end;
...
begin
  ...
  CheckHeal;
  if Weight < maxlim then FindGround;
end;
procedure LoseWeight;
begin
	Repeat
		if Weight > maxlim then DropWeight;
	until Weight < maxlim;
end;
...
begin
	...
	begin
		...
		grab(findtype(Items[1], ground), 0);
	end;
end;
begin
