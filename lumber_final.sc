...
var 
	...
	Items : array[1..14] of cardinal;
	Tools : array[1..3] of cardinal;
	Monstr : array[1..2] of cardinal;
	limit,h,b:integer;
	hatchet: cardinal;
procedure InitToolItem;
begin
    Tools[1] := $0F43; // hatchet
	...
	Items[13] :=$1078; // skin
	Items[14] :=$0F90; // deadwood
end;
procedure Bandages;
begin
	FindType(Tools[2],Backpack);
	if ((FindType(Tools[2],Backpack) = 0) or (FindQuantity < 15)) then
	begin
		if ((GetDistance(chest) > 1) or (GetDistance(chest) = -1)) then
		begin
	...
	begin
		...
		begin
			...
			begin
				...
				MoveItem(ItemID, 0,lootbox, 0, 0, 0);
				wait(350);
			end;
		end;
	end;
...
Begin
  ...
  begin
    ...
    UseObject(FindType(Tools[1],backpack));
    Wait(500);
...
begin
  ...
  begin
	...
	begin
		if FindNotoriety(Monstr[j],6) > 0 then
		begin
			AddToSystemJournal('Mob:',Monstr[j]);
			...
			begin
			  ...
			  begin
				...
				SetRunMountTimer(95);
				while GetDistance(Monstr[j])>0 do
				begin
					if not Paralyzed then NewMoveXY(GetX(Monstr[j]),GetY(Monstr[j]),true,1,true)
					else 
					begin
...
begin
	...
	begin
		...
		begin
			...
			wait(1000);
			AddToSystemJournal('Закончились топоры');
			if FindType(Tools[1],resbox) <> 0 then MoveItem(FindType(Tools[1],resbox), 0,backpack, 0, 0, 0);
		end;
	end;
...
begin
	...
	AddToSystemJournal('Total points to chop:',High(Tiles));
	limit:=(GetStr(self)*3)-100;
	CheckEquip;
	Bandages;
	Store;
	while not Dead do
