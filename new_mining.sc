...
procedure GoingHome;
begin
	if GetDistance(Krest) < 5 then
	begin
		NewMoveXY(X, Y, false, 1, true ); // пропиши тут точки, по которым можно обойти скелета
		NewMoveXY(X, Y, false, 1, true );
		NewMoveXY(X, Y, false, 1, true );
		NewMoveXY(X, Y, false, 1, true );
		NewMoveXY(X, Y, false, 1, true );
	end
	else if GetDistance(Krest) = -1 then NewMoveXY(HouseX, HouseY, false, 1, true )
	else NewMoveXY(HouseX, HouseY, false, 1, true );
end;
