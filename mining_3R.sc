...
Begin
  UOSay('''resend');
  if GetDistance(chest) = -1 then NewMoveXY(chestx, chesty, false, 1, true)
  else if GetDistance(chest) > 1 then NewMoveXY(chestx, chesty, false, 1, true);
  AddToSystemJOurnal('Открываем Пак!!!');
  UseObject(backpack);
  wait(500);
  AddToSystemJournal('Открываем Главный Сундук!!!');
  UseObject(chest);
  While LastContainer <> chest do UseObject(chest);
  wait(500);
  for g := 1 to 13 do
  begin
